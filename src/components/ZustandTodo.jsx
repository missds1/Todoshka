...
function ZustandUsers() {
  ...
  return (
    <div>
      <h2>Zustand Todo (Users)</h2>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "15px" }}>
        {users.map(user => (
          <Card
            key={user.id}
            title={editId === user.id ? (
              <Input value={editUserData.name} onChange={e => handleChange("name", e.target.value)} />
            ) : (
            ...
            actions={[
              editId === user.id ? (
                <Button type="primary" onClick={() => { editUser(editUserData.id, editUserData); setEditId(null); }}>Сохранить</Button>
              ) : (
